...
// std::string end_segment_name = your_tree.getSegments().end()->second.segment.getName();
    ...
    KDL::JntArray minLimit = KDL::JntArray(tree.getNrOfJoints());
    KDL::JntArray maxLimit = KDL::JntArray(tree.getNrOfJoints());
    shared_ptr<const urdf::Joint> jointTmp;
    int index = 0;
    for(unsigned int i=0;i<tree.getNrOfSegments();i++){
        if (tree.getSegment(i).getJoint().getType() != KDL::Joint::None) { 
            jointTmp = model.getJoint(tree.getSegment(i).getJoint().getName());
    ...
    // //Creation of the Kinematics solvers
    //         //Forward Kinematics Position solver
    //     KDL::TreeFkSolverPos_recursive fksolver(chain);		
    //         //Inverse Kinematics Velocity solver
    //     KDL::TreeIkSolverVel_pinv iksolver_v(chain);		
    //         // Inverse Kinematics Position solver with joint limits
    //         // Maximum 1000 iterations, stop at accuracy 1e-8
    //     KDL::TreeIkSolverPos_NR_JL iksolver_p(chain, minLimit, maxLimit, fksolver, iksolver_v, 10000, 1e-8);    
